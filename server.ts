...
if (cluster.isMaster) {
  ...
  app.set('views', join(DIST_FOLDER, 'application-client'));

  // Server static files from /application-client
  app.get('*.*', express.static(join(DIST_FOLDER, 'application-client'), {
    maxAge: '30d'
  }));
